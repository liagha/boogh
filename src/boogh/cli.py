...
def ride_group(sub, ride, login):
    ...
    for name, fn in [("status", ride.status), ("rating", ride.rating),
        ...
        token(leaf)
        leaf.set_defaults(func=fn)
    track = branch.add_parser("track")
    track.add_argument("--follow", type=int, default=0)
    track.add_argument("--timeout", type=int, default=600)
    token(track)
    track.set_defaults(func=ride.track)
